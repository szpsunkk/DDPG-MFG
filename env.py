...
LastEditor: skk
LastEditTime:
Discription: the environment of MEC to offload the resource, action = [r, b, o], state = [r, b, n, n * n]
'''
...
#####################  hyper parameters  ####################
LOCATION = "KAIST"  # data name
USER_NUM = 30  # user number
EDGE_NUM = 10  # edge number
LIMIT = 6  # the largest number offering of every edge server
MAX_EP_STEPS = 3000
TXT_NUM = 92  # the number of files in the data set
r_bound = 1e9 * 0.063  # Rn 1e9
b_bound = 1e9  # Rc 1e9
...
#####################  function  ####################
def trans_rate(user_loc, edge_loc):  # 传输速度
    B = BANDWIDTH
    P = POWER
...
# 初始化edge之间的带宽记录表
def BandwidthTable(edge_num):  # 带宽  edge_num* edge_num
...
def generate_state(two_table, U, E, x_min, y_min):
    # initial
    one_table = two_to_one(two_table)  # 这两个什么意思？
    S = np.zeros((len(E) + one_table.size + len(U) + len(U) * 2))
    # transform   S矩阵中包含（每个边缘服务器可用资源，每个连接可用带宽，每个用户卸载，每个用户的位置）  整合到一个矩阵中
    count = 0
    # available resource of each edge server   每个边缘服务器可用资源
    for edge in E:
        S[count] = edge.capability / (r_bound * 10)  # 0.1   平均每个edge的承载能力0.1   #这里或许可以改进，因为实际上不一定每个edge平均能力
        count += 1
    # available bandwidth of each connection    每个连接可用的带宽，可用的连接怎么算？
...
def proper_edge_loc(edge_num):  # 需要修改
    # 获得边缘服务器的位置
    # initial the e_l
...
def proper_edge_loc_beijing(edge_num):  # 需要修改
    # 获得边缘服务器的位置
    # initial the e_l
    e_l = np.zeros((edge_num, 2))
    # calculate the mean of the data
...
class UE():
    def __init__(self, user_id, data_num):
        ...
        now_sec = 0
        for line in f1:
            for sec in range(30):  # self.mob中每个(x, y)连续存储30次
    ...
    def request_update(self):
        ...
        # trans = trans_rate(self.loc, self.req.edge_loc)
        if self.req.state == 5:
            self.req.timer += 1
            trans = 0
        else:
            ...
            if self.req.state == 0:
                ...
                if self.req.u2e_size > 0:  # 判断是否传完
                    self.req.u2e_size -= trans_rate(self.loc, self.req.edge_loc)
                    trans = trans_rate(self.loc, self.req.edge_loc)

                else:
                    self.req.state = 2   # 处理进程
...
class TaskType():
    def __init__(self):
        # transmission
        self.req_u2e_size = 300 * 300 * 3 * 1000  # 用户到边缘服务器的服务大小300 * 300 * 3 * 1   byte
        self.process_loading = 300 * 300 * 3 * 4  # # 卸载大小     byte
        self.req_e2u_size = 4 * 4 + 20 * 4  # 边缘服务器到用户的服务大小  byte
        # migration
        self.migration_size = 2e9  # 迁移大小  实际带宽  byte/s

    def task_inf(self):  # 返回任务信息
...
class EdgeServer():
    def __init__(self, edge_id, loc):
        ...
        self.loc = loc
        self.capability = 1e9 * 0.063  # 所有edge server 的总承载能力    服务器最大处理能力 byte/s
        self.user_group = []
        self.limit = LIMIT
...
class priority_policy():  # 生成优先策略
    def generate_priority(self, U, E, priority):
        for user in U:
            # get a list of the offloading priority  卸载优先权
            dist = np.zeros(EDGE_NUM)
            for edge in E:
                dist[edge.edge_id] = np.sqrt(np.sum(np.square(user.loc[
                                                                  0] - edge.loc)))  # 求向量的距离公式：  np.sqrt(np.sum(np.square(vector1 - vector2))) # (x, y) - (x, y)
            dist_sort = np.sort(dist)  # 默认由小到大，所以距离越近，优先权越高
            for index in range(EDGE_NUM):
                priority[user.user_id][index] = np.argwhere(dist == dist_sort[index])[
                    0]  # np.argwhere返回非0的数组元组的索引，其中a是要索引数组的条件。
        return priority  # 10*10 数组

    def indicate_edge(self, O, U, priority):
        edge_limit = np.ones(
            (EDGE_NUM)) * LIMIT  # [4,4,4,4,4,4,4,4,4,4] 表示每个edge目前可以挂在的user数，为0的时候表示不能加入user了     # np.ones 返回一个全1的n维数组
        for user in U:
            for index in range(EDGE_NUM):  # 10*10的数组每一行第一列的数加入O[user.user_id]，若这个数加入了四次，看第二列的数
...
class Env():
    def __init__(self):
        ...
        self.E = []  # 边缘服务器矩阵
        # self.x_min, self.y_min = get_minimum_beijing()  # data表中最小的x, y
    ...
    def get_inf(self):
        # s_dim  状态state的存储大小
        self.reset()
        s = generate_state(self.table, self.U, self.E, self.x_min,
                           self.y_min)  # 状态包括edge可用资源、edge之间连接的带宽、需要卸载的用户、用户的位置（x,y)
        s_dim = s.size  # s状态的大小

        # a_dim   action维度  行动action的存储大小
        ...
        # maximum bandwidth    table的维度 edge*edge？  最大带宽
        b_bound = self.table[0][1]
        b_bound = b_bound.astype(np.float32)  # 对数据类型转换

        # task size  任务大小
        task = TaskType()
        task_inf = task.task_inf()  # 任务信息

        return s_dim, r_dim, b_dim, o_dim, r_bound, b_bound, task_inf, LIMIT, LOCATION, self.user_num, self.edge_num, BANDWIDTH, POWER
    ...
    def reset(self):
        ...
        self.priority = self.model.generate_priority(self.U, self.E,
                                                     self.priority)  # # 优先策略：10*10 数组  每一行代表一个user在各个edge服务器的优先顺序，数字越小，离得越近，优先权越高
    ...
    def ddpg_step_forward(self, a, r_dim, b_dim):
        ...
        self.table = BandwidthTable(self.edge_num)
        # release the resource   初始化边缘基站的资源为 6.3*e7
        for edge in self.E:
            edge.release()
        save_user = []
        trans = []
        # update the policy every second
        # resource update  a[r]回报
        self.R = a[:r_dim]
        # bandwidth update   a[r:r+b]带宽
        self.B = a[r_dim:r_dim + b_dim]
        # offloading update
        base = r_dim + b_dim
        for user_id in range(self.user_num):
            prob_weights = a[base:base + self.edge_num]
            # print("user", user_id, ":", prob_weights)    # 查看每个user网络的权重
            l = len(prob_weights)
            s = np.sum(prob_weights)
            for i in range(l):
                prob_weights[i] = prob_weights[i] / s
            action = np.random.choice(range(len(prob_weights)),
                                      p=prob_weights.ravel())  # select action w.r.t the actions prob  在这里随机选择action么？
            base += self.edge_num
            self.O[user_id] = action  # 观测当前每个用户卸载策略

        # request update
        user_num = 1
        for user in self.U:
            # update the state of the request
            trans_ = user.request_update()
            save_user.append([user_num, user.req.u2e_size])
            trans.append([user_num, trans_])
            # if user_num == 2:
            #     print(user.req.u2e_size)
            #     save_user.append(user.req.u2e_size)

            if user.req.timer >= 5:
                user.generate_request(self.O[user.user_id])  # offload according to the priority 初始化  重新产生request
        ...
        # return s_, r
        return generate_state(self.table, self.U, self.E, self.x_min, self.y_min), self.rewards, save_user, trans
    ...
    def text_render(self):
        ...
        """
        print("O:", self.O)
        for user in self.U:  # 打印每个user的loc、request state、edge serve
            print("user", user.user_id, "'s loc:\n", user.loc)
            print("request state:", user.req.state)
            print("edge serve:", user.req.edge_id)
        for edge in self.E:  # 打印每个edge的user_group, 即每个边缘服务器正在为哪些用户服务
            print("edge", edge.edge_id, "user_group:", edge.user_group)
        print("reward:", self.rewards)  # 打印reward
        print("=====================update==============================")
