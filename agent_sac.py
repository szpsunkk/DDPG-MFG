...
class GaussianPolicy(nn.Module):
    def __init__(self, state_dim, r_dim, b_dim, o_dim, hidden_dim, action_space=None):
        ...
        self.mean_linear = nn.Linear(hidden_dim, self.a_dim)
        self.log_std_linear = nn.Linear(hidden_dim, self.a_dim)
...
class SAC(object):
    def __init__(self, state_dim, action_dim, cfg, env_info):
        ...
        self.critic = QNetwork(state_dim, action_dim, cfg.hidden_dim).to(device=self.device)
        self.critic_optim = Adam(self.critic.parameters(), lr=cfg.lr)

        self.critic_target = QNetwork(state_dim, action_dim, cfg.hidden_dim).to(self.device)
        ...
        if self.policy_type == "Gaussian":
            ...
            self.policy = GaussianPolicy(state_dim, action_dim, cfg.hidden_dim).to(
                self.device)
            ...
            self.policy = DeterministicPolicy(state_dim, action_dim, cfg.hidden_dim).to(
                self.device)
            self.policy_optim = Adam(self.policy.parameters(), lr=cfg.lr)
    ...
    def save(self, env_name, suffix="", ckpt_path=None):
