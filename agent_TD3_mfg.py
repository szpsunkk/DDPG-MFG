...
class Critic_MFG(nn.Module):
    def __init__(self, state_dim, action_dim, action_mean_dim, hidden_dim, init_w=3e-3):
        ...
        # net 2
        self.linear6 = nn.Linear(state_dim + action_dim + action_mean_dim, hidden_dim)
    ...
    def Q1(self, state, action, action_mean):
        x = torch.cat([state, action, action_mean], 1)
        x = torch.abs(x)
        # net 1
        x1 = F.relu(self.linear1(x))
...
class TD3_MFG(object):
    def __init__(self, state_dim, action_dim, action_mean_dim, cfg, env_info):
        ...
        self.memory = ReplayBuffer_MFG(self.s_dim, self.a_dim)
        self.p = 0
    ...
    def update(self, cfg, i_ep, i_step):
        ...
        action_r_mean = torch.mean(action_r, 1).reshape(self.batch_size, 1)
        action_b_mean = torch.mean(action_b, 1).reshape(self.batch_size, 1)
        action_mean = torch.cat((action_r_mean, action_b_mean), 1)
        action_mean = torch.cat((action_mean, action_o), 1)
        ...
        with torch.no_grad():
            ...
            #
            next_action, next_action_mean = self.actor_target(next_state)

            # Compute the target Q value
            target_Q1, target_Q2 = self.critic_target(next_state, next_action, next_action_mean)
            target_Q = torch.min(target_Q1, target_Q2)
            target_Q = reward + self.gamma * target_Q

        # Get current Q estimates
        current_Q1, current_Q2 = self.critic(state, action, action_mean)

        # Compute critic loss
        critic_loss = F.mse_loss(current_Q1, target_Q) + F.mse_loss(current_Q2, target_Q)
        # if i_ep == self.p:
        # 	test_critic_loss = "critic loss" + np.str(i_ep)
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            # Compute actor loss
            action, action_mean, = self.actor(state)
            actor_loss = -self.critic.Q1(state, action, action_mean).mean()
            if i_ep == self.p:
                test_actor_loss = "actor loss" + np.str(i_ep)
