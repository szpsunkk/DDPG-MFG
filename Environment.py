...
class EdgeEnv:
    ...
    def reset(self, time, user_pos_distribution, user_pos_prob_distribution, user_job_arrive_prob):
        ...
        for i in range(self.num_user):
            new_user = User(self.user_cpu_speed, self.avg_job_cycles, self.SERVER_CPU_SPEED,
                            user_job_arrive_prob, self.user_job_arrive_rate,
                            np.random.choice(user_pos_distribution, p=user_pos_prob_distribution),
    ...
    def step(self, time, new_price):
        ...
        if self.buffer_using - 10 * self.SERVER_CPU_SPEED > 0:
            ...
            self.buffer_using = 0
        reward = 0
        pred_delay = 1 / (2 * self.MEC_job_finish_rate) + np.sqrt(
            new_price / (0.9 * self.MEC_job_finish_rate) + 1 / (4 * np.power(self.MEC_job_finish_rate, 2)))
