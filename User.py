...
class User:
    def __init__(self, cpu_speed, avg_job_cycle, server_cpu_speed,
        ...
        self.buffer_using = 0
        self.trans_speed = 360 / 8 * np.log2(
            1 + (self.trans_power * np.power(self.position, -1 * self.path_loss_exp)) / np.power(10, self.SNR / 10))
    ...
    def get_user_profit(self, price, delay):
        ...
        user_profit = self.time_weight * (local_time - edge_time) + self.energy_weight * (
                    local_power - edge_power) - price
        return user_profit
